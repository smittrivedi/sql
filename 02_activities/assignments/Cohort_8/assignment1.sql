...
GROUP BY vendor_id
ORDER BY times_rented DESC, vendor_id;
...
SELECT
    customer_id,
    STRFTIME('%m', purchase_datetime) AS month,
    STRFTIME('%Y', purchase_datetime) AS year
FROM customer_purchases;
...
FROM customer_purchases
WHERE STRFTIME('%Y', purchase_datetime) = '2022'
  AND STRFTIME('%m', purchase_datetime) = '04'
GROUP BY customer_id
ORDER BY total_spent_apr_2022 DESC, customer_id;
